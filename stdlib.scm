		...
		(letrec ((fold-left-loop 
		       (lambda (func base lst) 
				(
					if (null? lst) 
					base 
					(fold-left-loop func (func base (car lst)) (cdr lst))
				))
				))
		fold-left-loop
		)
...
(define reverse-assist
	(let ((null? null?) (car car) (cdr cdr) (cons cons))
		(letrec ((reverse-assist-loop 
			(lambda (lst acc)
				(
					if (null? lst)
					acc
					(reverse-assist (cdr lst) (cons (car lst) acc))
				)
			)))
		reverse-assist-loop)
...
(define reverse
	...
	(let ((reverse-assist reverse-assist))

			(lambda (lst)	(reverse-assist lst '()))
...
(define cons*
(let ((null? null?) (car car) (cdr cdr) (cons cons) (reverse reverse) (fold-right fold-right))
		(lambda (first . rest)
			(
				if (null? rest)
				first
				(
									let ((r (reverse rest)))
			(fold-right cons (car r) (cons first (reverse (cdr r))))
				)
			)
		)
)
